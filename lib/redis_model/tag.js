...
exports.setManyWordCount = function(tagNames, count) {
	console.log("setManyWordCount")
	var deferred = Q.defer();
	var current = 0,
		max = tagNames.length;
	open();
...
exports.getWordCount = function(tagName) {
	var deferred = Q.defer();
	open();
...
function getKey(tagName) {
	return "tagu_" + tagName;
}

function getWordCountKey(tagName) {
	return "tagWordCountu_" + tagName;
}
